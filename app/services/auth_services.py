...
def create_user_service(email, password):
    ...
    try:
        ...
        credential_id = create_credential_user_token_repo(email, hashed_password, hash_token(token_data.get("token")), token_data.get("token_expiry"))[0]
...
def verify_email_token_service(credential_id: str, token: str):
    try:
        ...
        print(token_data)
        is_token_valid = verify_token(token, token_data.get("hashed_token"))
        if not is_token_valid:
            raise ValueError("Could Not Validate token")
        if token_data.get("expire_at") < datetime.utcnow():
            is_used = True
            raise ValueError("Token have expired")
        if token_data.get("is_verified") == False  and token_data.get("is_used") == False:
            is_used = True
            is_verified = True
...
if __name__ == "__main__":
    token = 'YHjnw9OVf47C7iCCzENdnPJnlTKFCu7H2KiYOxjwgaI'
    hashed_token = '$argon2id$v=19$m=65536,t=3,p=4$/o6H9aavYVHFV1gqWybMXA$rf/WFIDAzlgFJIdBoUa3xm7346TQphj/BBdBOdKzQD4'
    if verify_token(token, hashed_token):
        print("true")
    else:
        print("false")